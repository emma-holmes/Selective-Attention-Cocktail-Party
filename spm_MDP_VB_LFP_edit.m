function [u,v,LFP_plot_x,LFP_plot_y] = spm_MDP_VB_LFP_edit(MDP,UNITS,f,SPECTRAL,filestr)
...
% UNITS(2,j) - time step
% (Note: will be in x and u as N element vector; goes through each state for
% first time step first, then each state for second time step, etc.)
%
% FACTOR     - hidden factor to plot                  [default: 1]
% SPECTRAL   - replace raster with spectral responses [default: 0]
%
% filestr    - String specifying a suffix to add to the end of output
%              filename [default: '', i.e. blank]
%
...
% Edited by Emma Holmes - added new LFP plot and output variables
...
%==========================================================================
figure;
plot(t,spm_cat(u), 'LineWidth', 1.5, 'Color', [.5, .5, .5]);
set(gca,'XTick',(1:(Ne*Nt))*Nb*dt);
format_graph(gca, 'Time (seconds)', 'Amplitude (a.u.)');
set(gcf, 'Position', [4, 254, 985, 325]);
LFP_plot_x = t;
LFP_plot_y = spm_cat(u);

% Save variables to file
%save(sprintf('LFPs_%s_%s', filestr)); % <- Uncomment to save variables
